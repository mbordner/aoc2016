...
func main() {
	...
	origValues := make([][]int, len(lines))
	for i, line := range lines {
		if reValues.MatchString(line) {
			matches := reValues.FindStringSubmatch(line)
			origValues[i] = []int{common.StrToA(matches[1]), common.StrToA(matches[2]), common.StrToA(matches[3])}
		}
	}

	values := make([][]int, 0, len(origValues))
	for y := 0; y <= len(origValues)-3; y += 3 {
		tvalues := [][]int{{}, {}, {}}
		for j := 0; j < 3; j++ {
			for i := 0; i < 3; i++ {
				tvalues[i] = append(tvalues[i], origValues[y+j][i])
			}
		}
